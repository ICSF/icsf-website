...
public class CssCompressor
{
	...
	protected String extractDataUrls(String css,
		...
		throws IOException
	{
		...
		while (m.find())
		{
			m.appendReplacement(sb, m.group(1).toLowerCase() + ":0 0" + m.group(
				2));
		}
		m.appendTail(sb);
		css = sb.toString();
		...
		while (m.find(index))
		{
			...
			if (isFilter)
			{
				...
				sb.append(m.group(1)).append('#').append(m.group(2)).append(m.group(
					3)).append(m.group(4)).append(m.group(5)).append(m.group(6)).append(m.group(
					7));
			}
			else
			{
				...
				else
				{
					...
					sb.append('#').append((m.group(2) + m.group(3) + m.group(4) + m.group(
						5) + m.group(6) + m.group(7)).toLowerCase());
				}
			}
		...
		sb = new StringBuffer();
		p = Pattern.compile(
			"(?i)(border|border-top|border-right|border-bottom|border-right|outline|background):none(;|})");
		m = p.matcher(css);
		while (m.find())
		{
